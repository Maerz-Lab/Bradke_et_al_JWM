...
# The "MODEL CODE" is used to evaluate if the simulated increases in survival would be detected using standard monitoring methods if data were collected at eight sites.
...
write.csv(Simulation,"Simulation_params8.csv")
...
ch.sites.8 <- array(dim=c(iter*n.sites))
ch.sites.8 <- as.data.frame(ch.sites.8)
...
for (k in 1:iter){ 
  ...
  ch.sites.8[(cnss2[k]+1):cnss2[k+1],] <- ch.sites
  ch.sites.8[(cnss2[k]+1),2:16] <- Simulation[ch.sites[1],]
  ch.sites.8[(cnss2[k]+2),2:16] <- Simulation[ch.sites[2],]
  ch.sites.8[(cnss2[k]+3),2:16] <- Simulation[ch.sites[3],]
  ch.sites.8[(cnss2[k]+4),2:16] <- Simulation[ch.sites[4],]
  ch.sites.8[(cnss2[k]+5),2:16] <- Simulation[ch.sites[5],]
  ch.sites.8[(cnss2[k]+6),2:16] <- Simulation[ch.sites[6],]
  ch.sites.8[(cnss2[k]+7),2:16] <- Simulation[ch.sites[7],]
  ch.sites.8[cnss2[k+1],2:16] <- Simulation[ch.sites[8],]
  
  CH1 <- as.data.frame(CH[[ch.sites[1]]])
  CH2 <- as.data.frame(CH[[ch.sites[2]]])
  CH3 <- as.data.frame(CH[[ch.sites[3]]])
  CH4 <- as.data.frame(CH[[ch.sites[4]]])
  CH5 <- as.data.frame(CH[[ch.sites[5]]])
  CH6 <- as.data.frame(CH[[ch.sites[6]]])
  CH7 <- as.data.frame(CH[[ch.sites[7]]])
  CH8 <- as.data.frame(CH[[ch.sites[8]]])
  
  group[[k]] <- c(rep(1,dim(CH1)[1]),rep(2,dim(CH2)[1]),rep(3,dim(CH3)[1]),rep(4,dim(CH4)[1]),rep(5,dim(CH5)[1]),rep(6,dim(CH6)[1]),rep(7,dim(CH7)[1]),rep(8,dim(CH8)[1]))
  
  CH.comb[[k]] <- as.matrix(rbind(CH1,CH2,CH3,CH4,CH5,CH6,CH7,CH8))
  
  tide[[k]] <- rbind(tide.site[ch.sites[1],],tide.site[ch.sites[2],],tide.site[ch.sites[3],],tide.site[ch.sites[4],],tide.site[ch.sites[5],],tide.site[ch.sites[6],],tide.site[ch.sites[7],],tide.site[ch.sites[8],])
}
...
file.samps <- paste("./Samples8_60NS/Simulation_samples",file.num,".csv",sep = "")
file.summs <- paste("./Summary8_60NS/Simulation_summary",file.num,".csv",sep = "")
...
write.csv(Simulation.results,"Simulation_results8.csv")

colnames(ch.sites.8) <- c("sim",colnames(Simulation))
write.csv(ch.sites.8,"Simulation_CH_params8.csv")
