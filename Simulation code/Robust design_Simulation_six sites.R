...
# The "MODEL CODE" is used to evaluate if the simulated increases in survival would be detected using standard monitoring methods if data were collected at six sites.
...
write.csv(Simulation,"Simulation_params6.csv")
...
ch.sites.6 <- array(dim=c(iter*n.sites))
ch.sites.6 <- as.data.frame(ch.sites.6)
...
for (k in 1:iter){ 
  ...
  ch.sites.6[(cnss2[k]+1):cnss2[k+1],] <- ch.sites
  ch.sites.6[(cnss2[k]+1),2:16] <- Simulation[ch.sites[1],]
  ch.sites.6[(cnss2[k]+2),2:16] <- Simulation[ch.sites[2],]
  ch.sites.6[(cnss2[k]+3),2:16] <- Simulation[ch.sites[3],]
  ch.sites.6[(cnss2[k]+4),2:16] <- Simulation[ch.sites[4],]
  ch.sites.6[(cnss2[k]+5),2:16] <- Simulation[ch.sites[5],]
  ch.sites.6[cnss2[k+1],2:16] <- Simulation[ch.sites[6],]
  ...
  group[[k]] <- c(rep(1,dim(CH1)[1]),rep(2,dim(CH2)[1]),rep(3,dim(CH3)[1]),rep(4,dim(CH4)[1]),rep(5,dim(CH5)[1]),rep(6,dim(CH6)[1]))
  
  CH.comb[[k]] <- as.matrix(rbind(CH1,CH2,CH3,CH4,CH5,CH6))
  
  tide[[k]] <- rbind(tide.site[ch.sites[1],],tide.site[ch.sites[2],],tide.site[ch.sites[3],],tide.site[ch.sites[4],],tide.site[ch.sites[5],],tide.site[ch.sites[6],])
}
...
file.samps <- paste("./Samples6/Simulation_samples",file.num,".csv",sep = "")
file.summs <- paste("./Summary6/Simulation_summary",file.num,".csv",sep = "")
...
write.csv(Simulation.results,"Simulation_results6.csv")

colnames(ch.sites.6) <- c("sim",colnames(Simulation))
write.csv(ch.sites.6,"Simulation_CH_params6.csv")
